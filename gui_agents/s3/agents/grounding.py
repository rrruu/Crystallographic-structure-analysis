...
class OSWorldACI(ACI):
    ...
    def generate_coords(self, ref_expr: str, obs: Dict) -> List[int]:
        """
        Generate (x,y) coordinates from the grounding model.

        Minimal patch goals:
        1) Strongly constrain the grounding model to output ONLY one coordinate pair
           in a strict format: <point>x y</point>
        2) Strictly parse that format; if not present, raise to avoid mis-parsing
           random numbers in free-form text.
        """

        # Reset the grounding model state
        self.grounding_model.reset()

        # Stronger, explicit, strict output contract for the grounding model
        prompt = (
            f"Query:{ref_expr}\n"
            "Return EXACTLY one point in the following format and NOTHING ELSE:\n"
            "<point>x y</point>\n"
            "Where x and y are integers.\n"
            "Do not include any other text, explanations, punctuation, or additional numbers.\n"
            "If uncertain, still output your best guess in the required format.\n"
        )
        ...
        # # Strict parse: only accept <point>int int</point>
        # m = re.search(r"<point>\s*(\d+)\s+(\d+)\s*</point>", response)
        # if not m:
        #     # Do NOT fall back to grabbing arbitrary numbers; that's what causes clicks to fly off.
        #     raise ValueError(
        #         f"Grounding response missing <point>x y</point>. Got: {response[:200]!r}"
        #     )
        #
