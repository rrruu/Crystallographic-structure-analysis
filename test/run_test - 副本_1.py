...
# 屏幕与接地配置 (根据你的 1920x1080 屏幕与 UI-TARS 模型特性)
SCREEN_WIDTH = 1920
SCREEN_HEIGHT = 1080
# UI-TARS 坐标系统通常为 1000x1000
GROUNDING_DIM = 1000

# ================= 2. 智能化任务目标模块 =================
# 将 36 步指令压缩为 6 个核心目标，利用 Agent 的推理能力自主寻找路径
SUB_TASKS = [
    {
    ...
    {
        "goal": "在 ATOMS 软件中点击 'Open'，请先进入 'C:\\Users\\Lenovo\\Desktop\\演示5' 路径，确认正确进入路径后，加载该路径下的数据文件 'ICSD_CollCode103903.str'，直到看到结构图显示在主窗口中。",
        "desc": "数据加载",
    },
    {
    ...
    {
        "goal": "保存结果：通过 'File' 菜单进入 'Save Graphics Window'，选择 '.BMP' 格式。在弹出的保存对话框中，请先进入 'D:\\code\\python\\AgentS_ATOMS\\utils\\bmp' 路径。确认正确进入路径后，点击右下角的 '保存' 按钮完成保存图像结果。",
        "desc": "结果导出",
    },
]
