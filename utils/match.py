...
if src is None:
    raise FileNotFoundError(f"读不到原始图片: {SRC_PATH}")
if tgt is None:
    raise FileNotFoundError(f"读不到答案图片: {TGT_PATH}")
...
if tgt.ndim == 2:
    tgt_bgr = cv2.cvtColor(tgt, cv2.COLOR_GRAY2BGR)
else:
    tgt_bgr = tgt[:, :, :3]
...
def transform_points(
    pts, scale=1.0, angle_deg=0.0, translation=(0.0, 0.0), center=None
):
...
def icp_translation(src_trans, max_iter=20, thresh=15):
    ...
    for _ in range(max_iter):
        ...
        pts += delta
        trans += delta
        if float(np.linalg.norm(delta)) < 0.1:
            break
...
for ang in np.linspace(-180, 180, 1441):
    ...
    if best is None or meddist < best[0]:
        best = (meddist, float(ang), delta, pts_aligned)
...
for dy in range(-30, 31, 2):
    for dx in range(-30, 31, 2):
        ...
        if hit > best_hit or (hit == best_hit and s < best_sum):
            best_hit, best_sum, best_dx, best_dy = hit, s, dx, dy
...
for (px, py), m in zip(pts_final, matched):
    if m:
        cv2.circle(overlay2, (int(round(px)), int(round(py))), 3, (0, 255, 255), -1)
...
if len(yellow_pts) > 0:
    ...
    if cnts:
        ...
        # 保存两个 CSV
        pd.DataFrame(pts_in_src, columns=["src_x", "src_y"]).to_csv(
            str(OUT_BLUE_SRC_CSV), index=False
        )
        pd.DataFrame(pts_in_src_2, columns=["src_x", "src_y"]).to_csv(
            str(OUT_BLUE_SRC_CSV_2), index=False
        )
        # ----------------------------------------
...
print(
    f"   - [CSV] 原始图坐标(保留旋转版本): {OUT_BLUE_SRC_CSV_2}  <-- NEW!"
)  # 追加信息
...
print(
    f"   - 边界蓝色点数: {len(blue_pause_pts) if 'blue_pause_pts' in locals() else 0}"
)
print("=" * 50 + "\n")
